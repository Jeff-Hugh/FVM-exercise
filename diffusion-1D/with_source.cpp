...
int main()
{
	const double L = 0.02; // total length
	const double A = 1; // sectional area
	const double TA = 100; // boundary condition A
	const double TB = 200; // boundary condition B
	const double k = 0.5; // thermal conductivity
	const double q = 1e6;	// thermal source, W/m^2
	...
	diag_data[0] = 3*temp;
	e_data[0] = -temp;
	b_data[0] = 2*temp*TA + q*A*dx;

	// B boundary, i = N-1
	f_data[N-2] = -temp;
	diag_data[N-1] = 3*temp;
	b_data[N-1] = 2*temp*TB + q*A*dx;
